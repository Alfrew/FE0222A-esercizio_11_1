...
import { loadDB, getPosts } from "../service/post.service";
    ...
    <div class="container mt-5">
      <div *ngFor="let post of posts">
        <div class="mb-3" *ngIf="post.active" [ngClass]="changeCol(post)">
          <h5 class="card-header">Active post</h5>
          <div class="card-body">
            <h5 clas>{{ post.title }}</h5>
            <p class="card-text">{{ post.body }}</p>
            <button class="btn btn-secondary form-control" (click)="changeStatus(post)">Disattiva</button>
          </div>
        </div>
      </div>
    </div>
